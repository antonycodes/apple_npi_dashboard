    ...
    <main className="min-h-screen bg-[#f7f6f3] px-4 py-6 text-neutral-800 sm:px-6">
      <div className="mx-auto max-w-6xl">
        <header className="flex flex-wrap items-start justify-between gap-4 border-b border-neutral-200 pb-5">
          <div><a href={mock ? '/admin/logs' : '/app'} className="inline-flex items-center gap-2 text-sm font-bold text-neutral-500 hover:text-neutral-900"><ArrowLeftIcon className="h-4 w-4" /> {mock ? 'Nhật ký thật' : 'Quản trị'}</a><h1 className="mt-4 text-2xl font-black tracking-tight text-neutral-950">Nhật ký vận hành{mock ? ' · Mock data' : ''}</h1><p className="mt-1 text-sm text-neutral-500">Theo dõi hành trình khách qua tất cả các khâu.</p>{!mock && <a href="/system-control" className="mt-3 inline-flex min-h-10 items-center rounded-lg border border-red-200 bg-red-50 px-3 text-sm font-bold text-red-800 hover:bg-red-100">Mở Kiểm soát hệ thống</a>}</div>
