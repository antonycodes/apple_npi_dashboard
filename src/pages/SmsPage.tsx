      ...
      <main className="mx-auto w-full max-w-[1500px] px-4 py-4 md:px-6 md:py-6">
        ...
        <div className="mt-4 grid grid-cols-4 gap-2 sm:grid-cols-8 lg:grid-cols-10 xl:grid-cols-[repeat(16,minmax(0,1fr))]">
          {numbers.map((number) => {
            ...
            const smsPendingReport = smsNotification
              ? /^(Đang chờ báo cáo|API đã tiếp nhận SMS - chờ báo cáo)$/i.test(smsNotification.trim())
              : false;
            const smsDotTone = smsPendingReport
              ? 'bg-amber-400'
              : smsNotification
                ? (smsSuccess ? 'bg-emerald-500' : 'bg-red-500')
                : 'bg-blue-700';
            return (
              <button
                key={stt}
              ...
              className={`relative aspect-square min-h-12 rounded-xl text-lg font-black shadow-sm transition focus-visible:outline-none focus-visible:ring-4 focus-visible:ring-brand/25 enabled:hover:-translate-y-0.5 enabled:hover:shadow-md disabled:cursor-not-allowed sm:min-h-14 sm:text-xl ${sttTone(journey, now, settings.leadtimeMinutes, settings.warningMinutesBefore, onlyPendingConsult)} ${smsPendingReport ? 'ring-2 ring-amber-400 ring-offset-1' : smsNotification ? (smsSuccess ? 'ring-2 ring-emerald-500 ring-offset-1' : 'ring-2 ring-red-500 ring-offset-1') : smsRequested ? 'ring-2 ring-blue-400 ring-offset-1' : ''}`}
              >
                {stt.padStart(2, '0')}
